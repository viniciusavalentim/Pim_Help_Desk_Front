...
export default function FloatingChat() {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    const currentActiveChat = findChatsQuery?.conversations.find(
      (c) => c.id === activeChatId
    );
    try {
  ...
  const activeChat = findChatsQuery?.conversations.find(
    (chat) => chat.id === activeChatId || chat.id.startsWith("temp-id")
  );
  ...
  console.log(findChatsQuery)
  return (
    <>
      <div
        ref={chatRef}
        className={`fixed bottom-24 right-6 w-96 bg-white rounded-2xl shadow-2xl transition-all duration-300 ease-in-out z-50 ${
          isOpen
      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-t-2xl p-4 flex items-center gap-2">
          ...
          <div className="w-12 h-12 rounded-full flex items-center justify-center">
            <img src="/pass.png" width={100} height={100} alt="Logo" />
          </div>
          <div className="flex-1">
            <h3 className="text-white font-semibold">
              {view === "list"
                ? "Sabiá | Central de Atendimento"
                : activeChat?.title
                ? activeChat.title.slice(0, 40) + "..."
                : "Assistente Virtual"}
            </h3>
            <p className="text-blue-100 text-sm">
              {view === "list" ? "Suas conversas" : "Online - Resposta rápida"}
            </p>
          </div>
          {view === "list" && (
            <button
          ...
          >
            {view === "list" ? (
              <div className="space-y-3">
                {isLoadingChats ? (
                  <Loader2 className="mx-auto my-12 h-8 w-8 animate-spin text-blue-500" />
                ) : !findChatsQuery || findChatsQuery?.conversations.length === 0 ? (
                  ...
                  findChatsQuery?.conversations.map((chat) => (
                    ...
                    >
                      <h4 className="font-semibold text-gray-800 mb-1 group-hover:text-blue-600 break-words truncate">
                        {chat.title}
                      </h4>
        ...
        {view === "chat" && (
          <form
            ...
            className="p-4 bg-white rounded-b-2xl border-t"
          >
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 ease-in-out z-50 hover:scale-110 ${
          isOpen
